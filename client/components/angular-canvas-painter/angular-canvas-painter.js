...
// (function(module) {
// try {
//   module = angular.module('styley2App');
// } catch (e) {
...
//     '<ul class="pwColorSelector"><li ng-repeat="color in colorList track by $index" class="pwColor" ng-class="{\'active\': (selectedColor === color)}" ng-click="setColor(color)"></li></ul>');
// }]);
// })();


angular.module('styley2App').run(['$templateCache', function($templateCache) {
  $templateCache.put('../templates/canvas.html',
    '<div class="pwCanvasPaint" style="position:relative"></div>');
}]);
angular.module('styley2App').run(['$templateCache', function($templateCache) {
  ...
  .directive('pwCanvas', ["$timeout", function ($timeout) {
    return {
      ...
      link: function postLink(scope, elm) {
        ...
        options.undoEnabled = options.undoEnabled || true;
        ...
        //set canvas size
        $timeout(function () {
          canvas.width = canvasTmp.width = elm.parent().parent().width();
          canvas.height = canvasTmp.height = elm.parent().parent().height();
        ...
        var activeCanvas
        scope.$watch('options.active', function(active) {
          activeCanvas=active
        });
